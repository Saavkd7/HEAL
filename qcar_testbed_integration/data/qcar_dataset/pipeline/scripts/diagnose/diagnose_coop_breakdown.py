...
cannot tell the two apart.
    ...
    python qcar_dataset/pipeline/scripts/diagnose/diagnose_coop_breakdown.py \
...
import wall_geometry  # noqa: E402
from build_coop_train_val_dataset import (  # noqa: E402
    STATIC_TARGETS, SCALE, WALLS, apply_marker_offset, truly_visible_to_agent,
)
...
def main():
    ...
    with torch.inference_mode():
        for idx, batch_data in enumerate(loader):
            ...
            # build_coop_train_val_dataset.py's own dataset-building logic,
            # user-decided 2026-09-22: always go by real geometry, not a
            # fixed WALL_BLOCKED pair fact) -- raw (uncorrected) positions,
            # matching that script's own convention.
            vis3_to_1 = False if wall_geometry.blocked_by_wall(
                (ego["x"], ego["y"]), (STATIC_TARGETS["3"]["x"], STATIC_TARGETS["3"]["y"]), WALLS
            ) else truly_visible_to_agent(
            ...
            vis4_to_2 = False if wall_geometry.blocked_by_wall(
                (peer["x"], peer["y"]), (STATIC_TARGETS["4"]["x"], STATIC_TARGETS["4"]["y"]), WALLS
            ) else truly_visible_to_agent(
                targets_world["4"]["x"], targets_world["4"]["y"], targets_world["4"]["z"],
