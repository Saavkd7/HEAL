...
opencood/ itself is edited; QCar-specific behaviour (1-camera loader, real
camera extrinsic) comes from plugins imported by qcar/config.py, matching the
"port, don't modify" discipline used for every other HEAL/QuantV2X fork in
this project.

WHAT IS CONFIGURABLE, AND WHERE
--------------------------------
Every default is in qcar/conf.json (train_* keys + the shared module keys).
Model, loss and fusion are chosen by the experiment yaml and can be swapped
without code changes -- see qcar/README.md:
    python qcar/train.py                                   # conf.json defaults
    python qcar/train.py -y qcar/configs/camera_attfuse_pretrain.yaml
    python qcar/train.py --set model.args.fusion_method=max
    python qcar/train.py --plugins qcar.patches.patch_1cam_loader
...
1. Stock train.py unconditionally shells out to inference.py at the end,
   ...
   lean, open3d-free qcar/eval.py instead (see that file).
...
def ego_modalities(hypes):
    """'m2' or 'm1&m2' -> ['m2'] / ['m1', 'm2']; models name their
    per-modality submodules encoder_<m>, backbone_<m>, ..."""
    ego = (hypes.get("heter", {}).get("ego_modality")
           or hypes["model"]["args"].get("ego_modality"))
    if not ego:
        raise SystemExit("no heter.ego_modality in the yaml; set "
                         "_qcar_freeze_module / _qcar_encoder_initialization_prefixes")
    return ego.split("&")


def freeze_trunk_paths(hypes):
    """Which image trunk(s) _qcar_freeze_encoder_blocks applies to: the yaml's
    _qcar_freeze_module (str or list, attribute path from the model), else the
    lift_splat_shoot trunk of every ego modality."""
    paths = hypes.get("_qcar_freeze_module")
    if paths:
        return [paths] if isinstance(paths, str) else list(paths)
    return ["encoder_%s.camencode.trunk" % m for m in ego_modalities(hypes)]
...
def get_submodule(model, dotted):
    ...
    for part in dotted.split("."):
        if not hasattr(node, part):
            raise SystemExit("model has no submodule %r (while resolving %r); "
                             "set _qcar_freeze_module in the yaml" % (part, dotted))
        node = getattr(node, part)
    return node


def load_initial_weights(hypes, model, n_train_frames):
    """Apply _qcar_pretrained_checkpoint (whole model, must match exactly) or,
    when that is null, _qcar_encoder_initialization_checkpoint (only tensors
    under _qcar_encoder_initialization_prefixes). Shared with qcar/zoo.py."""
    ...
    if "_qcar_pretrained_checkpoint" in hypes and not pretrained:
        ...
        if encoder_pretrained:
            encoder_path = config.repo_path(encoder_pretrained)
            ...
            prefixes = tuple(hypes.get(
                ...
                ["encoder_%s." % m for m in ego_modalities(hypes)],
            ))
            ...
            print("! every module outside %s starts randomly initialized." % (prefixes,))
        else:
...
def parse_args():
    ...
    ap.add_argument("--hypes_yaml", "-y", default=None,
                    help="experiment yaml (default: conf.json train_hypes_yaml)")
    ap.add_argument("--model_dir", default="",
                    help="resume from this run dir instead of training from scratch "
                         "(its config.yaml replaces --hypes_yaml, as in HEAL)")
    ap.add_argument("--batch_size", type=int, default=conf["train_batch_size"],
                    help="micro-batch; 1 is verified safe on the 3.68 GiB QCar GPU")
    ap.add_argument("--accumulation_steps", type=int,
                    default=conf["train_accumulation_steps"],
                    help="gradient accumulation steps")
    ap.add_argument("--gradient_clip", type=float, default=conf["train_gradient_clip"])
    ap.add_argument("--amp", dest="amp", action="store_true",
                    help="CUDA mixed precision on (default: conf.json train_amp)")
    ap.add_argument("--no_amp", dest="amp", action="store_false",
                    help="CUDA mixed precision off")
    ap.set_defaults(amp=conf["train_amp"])
    ap.add_argument("--epoches", type=int, default=conf["train_epoches"],
                    help="override train_params.epoches from the yaml")
    ap.add_argument("--seed", type=int, default=conf["train_seed"])
    ap.add_argument("--num_workers", type=int, default=conf["train_num_workers"])
    ...
    opt = ap.parse_args()
    opt.hypes_yaml = (config.cli_path(opt.hypes_yaml) if opt.hypes_yaml
                      else conf.path_of("train_hypes_yaml"))
    opt.model_dir = config.cli_path(opt.model_dir)
    return opt, conf


def main():
    opt, conf = parse_args()
    # yaml dataset/checkpoint paths are relative to the repo root
    ...
    hypes["_qcar_amp_requested"] = opt.amp
    ...
    train_loader = DataLoader(train_ds, batch_size=opt.batch_size, num_workers=opt.num_workers,
                              collate_fn=train_ds.collate_batch_train, shuffle=True,
                              pin_memory=True, drop_last=False)
    val_loader = DataLoader(val_ds, batch_size=opt.batch_size, num_workers=opt.num_workers,
    ...
    if freeze_n > 0:
        for trunk_path in freeze_trunk_paths(hypes):
            trunk = get_submodule(model, trunk_path)
            ...
            for block in trunk._blocks[:freeze_n]:
                for param in block.parameters():
                    param.requires_grad = False
            print("Froze stem + first %d/%d EfficientNet blocks of %s" %
                  (freeze_n, len(trunk._blocks), trunk_path))
        frozen = sum(p.numel() for p in model.parameters() if not p.requires_grad)
        trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print("  %d params frozen, %d params trainable" % (frozen, trainable))
    ...
    if opt.model_dir:
        ...
        load_initial_weights(hypes, model, len(train_ds))
    ...
    if hypes.get("test_dir"):
        print("Run qcar/eval.py --model_dir %s --split test ONLY ONCE, at the very end." % saved_path)
    else:
